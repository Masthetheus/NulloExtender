...
./nullo_extender <nullomer_file> <gc_max> <homopolymer_max> <number_of_seeds>
...
./nullo_extender data/subtilis_result 55 3 100

This invocation reads the subtilis_result index, selects 100 seeds with a GC content not exceeding 55 percent, and rejects any seed containing homopolymers longer than three bases. The extended sequences are written to standard output. Writing to output file is currently at work.


## 4. Algorithmic Overview

The program proceeds through four consecutive phases.
...
Accepted seeds are extended from length k to length k + 5 by sequential random base addition. At each extension step, the algorithm attempts to choose a base that does not violate the GC and homopolymer constraints for the extended sequence. If the initially drawn base violates a constraint, up to twenty alternative bases are tested. If no valid base is found, the least harmful base (i.e., one that does not increase the homopolymer run) is accepted, and a warning is issued.


5. Input File Format

The program expects the binary format produced by the companion nullomer counter. The file structure is as follows:

Byte offset 0-5: length 6 bytes, content: Magic header / reserved space.
Byte offset 6-13: length 8 bytes, content: Parameters: k (uint16), half_k (uint16), and two reserved uint16 fields.
Byte offset 14-15: length 2 bytes, content: Codes: byte_size (uint8) and counter_size (uint8).
Byte offset 16-end: length variable, content: Records, each consisting of a prefix (byte_size bytes), a counter (counter_size bytes), and an array of null_count suffixes (byte_size bytes each).

The index is assumed to be uncompressed in memory. Users working with large eukaryotic genomes should ensure that sufficient RAM is available.
...
./maw_extension data/hg38.nullomer 55 3 100 > selected_sequences.txt
